...
const btnClick = document.querySelector('#check-btn');
...
function ClickHandler() {
  let bdayString = userInput.value;

  if (bdayString === '') result.innerText = 'Enter a date and Check again!';
  else {
    if (bdayString !== '') {
      ...
      if (isPalindrome)
        result.innerText =
          'Woo Hoo! Your birthday is a palindrome! Congraaats! 🥳';
      else {
        let [prvCounter, prvDate] = prevPalindromeDate(date);
        let [nxtCounter, nxtDate] = nextPalindromeDate(date);
        if (prvCounter < nxtCounter)
          result.innerText = `Your birthday is NOT a palindrome, but the next palindrome date was ${prvCounter} days before on ${prvDate.day}-${prvDate.month}-${prvDate.year} 😀`;
        else if (prvCounter > nxtCounter)
          result.innerText = `Your birthday is NOT a palindrome, but the next palindrome date is ${nxtCounter} days later on ${nxtDate.day}-${nxtDate.month}-${nxtDate.year} 😀`;
      }
    }
  }
}
